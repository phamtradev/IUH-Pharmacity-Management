...
public class NhanVienDAO implements DAOInterface<NhanVien, String> {
    ...
    private final String SQL_THEM = 
            "INSERT INTO NhanVien (maNhanVien, tenNhanVien, diaChi, soDienThoai, email, vaiTro) VALUES (?, ?, ?, ?, ?, ?)";
    
    private final String SQL_CAP_NHAT = 
            "UPDATE NhanVien SET tenNhanVien = ?, diaChi = ?, soDienThoai = ?, email = ?, vaiTro = ? WHERE maNhanVien = ?";
    ...
    private final String SQL_TIM_THEO_TU_KHOA = 
            "SELECT * FROM NhanVien WHERE tenNhanVien LIKE ? OR soDienThoai LIKE ? OR email LIKE ?";
